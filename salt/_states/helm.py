...
def release_present(
    ...
    if "helm.status" not in __salt__:
        ...
        global_kv={}
        if 'kubeconfig' in kvflags:
            global_kv['kubeconfig'] = kvflags['kubeconfig']

        release_old_status = __salt__["helm.status"](release=name, namespace=namespace, kvflags=global_kv)
        if isinstance(release_old_status, dict):
            ...
            if isinstance(release_upgrade, bool) and release_upgrade:
                release_cur_status = __salt__["helm.status"](
                    release=name, namespace=namespace, kvflags=global_kv
                )
