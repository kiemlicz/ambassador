...
{%- set cmd = "kubeadm join --token " ~ tokens[main_master_id]['stdout'] ~ " " ~ ips[main_master_id][0] ~ ":" ~ kubernetes_network.nodes.apiserver_port ~ " --discovery-token-ca-cert-hash sha256:" ~ hashes[main_master_id] ~ " --control-plane --certificate-key " ~ cert_key[main_master_id] %}
