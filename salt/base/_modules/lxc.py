...
def _network_conf(lxc_version, conf_tuples=None, **kwargs):
    '''
    ...
    if not conf_tuples:
        conf_tuples = []
    old = _get_veths(conf_tuples, lxc_version)
    if not old:
        old = {}
    ...
    ifs.sort()
    gateway_set = False
    for idx, dev in enumerate(ifs, start=0):
        args = nicp.get(dev, {})
        ...
        if disable:
            continue
        net_key = _network_key_prefix(lxc_version, idx)
        mac = opts.get('mac',
                       opts.get('hwaddr',
                                args.get('mac',
                                         args.get('hwaddr', ''))))
        ...
        infos = salt.utils.odict.OrderedDict([
            ('{0}.type'.format(net_key), {
                'test': not type_,
                'value': type_,
                'old': old_if.get('{}.type'.format(net_key)),
                'default': 'veth'}),
            ('{0}.name'.format(net_key), {
                'test': False,
                'value': dev,
                'old': dev,
                'default': dev}),
            ('{0}.flags'.format(net_key), {
                'test': not flags,
                'value': flags,
                'old': old_if.get('{}.flags'.format(net_key)),
                'default': 'up'}),
            ('{0}.link'.format(net_key), {
                'test': not link,
                'value': link,
                'old': old_if.get('{}.link'.format(net_key)),
                'default': search_lxc_bridge()}),
            ('{0}.hwaddr'.format(net_key), {
                'test': not mac,
                'value': mac,
                'old': old_if.get('{}.hwaddr'.format(net_key)),
                'default': salt.utils.network.gen_mac()}),
            ('{0}.ipv4'.format(net_key), {
                'test': not ipv4,
                'value': ipv4,
                'old': old_if.get('{}.ipv4', ''),
                'default': None}),
            ('{0}.ipv6'.format(net_key), {
                'test': not ipv6,
                'value': ipv6,
                'old': old_if.get('{}.ipv6', ''),
                'default': None})])
        ...
        for key, val in six.iteritems(args):
            ...
            if key in [
                ...
                continue
            ret.append({'{0}.{1}'.format(net_key, key): val})
        ...
        if gateway is not None and not gateway_set:
            ret.append({'{0}.ipv4.gateway'.format(net_key): gateway})
    ...
    if gateway is not None and not gateway_set:
        ret.append({'lxc.network.ipv4.gateway'.format(_network_key_prefix(lxc_version, 0)): gateway})
        # only one network gateway ;)
        # for first ifc
        gateway_set = True

    new = _get_veths(ret, lxc_version)  # fixme confirm output is consistent
    # verify that we did not loose the mac settings
    for idx, iface in enumerate([a for a in new], start=0):
        net_key = _network_key_prefix(lxc_version, idx)
        ndata = new[iface]
        nmac = ndata.get('{0}.hwaddr'.format(net_key), '')
        ntype = ndata.get('{0}.type'.format(net_key), '')
        omac, otype = '', ''
        if iface in old:
            odata = old[iface]
            omac = odata.get('{0}.hwaddr'.format(net_key), '')
            otype = odata.get('{0}.type'.format(net_key), '')
        ...
        if not ntype:
            ntype = 'veth'
        new[iface]['{0}.type'.format(net_key)] = ntype
        if omac and not nmac:
            new[iface]['{0}.hwaddr'.format(net_key)] = omac
...
def _config_list(conf_tuples=None, only_net=False, **kwargs):
    ...
    if not only_net:
        ...
        for k, val in six.iteritems(default_data):
            ret.append({k: val})
    net_datas = _network_conf(lxc_version=version(), conf_tuples=conf_tuples, **kwargs)
    ret.extend(net_datas)
    return ret


def _network_key_prefix(lxc_version, idx=None):
    if lxc_version < _LooseVersion('3.0'):
        return "lxc.network"
    if idx is None:
        return "lxc.net"
    else:
        return "lxc.net.{0}".format(idx)


def _get_veths(net_data, lxc_version):
    '''
    Parse the nic setup inside lxc conf tuples back to a dictionary indexed by
    network interface
    '''
    if isinstance(net_data, dict):
        net_data = list(net_data.items())  # fixme does it preserve netdata order? is it ordereddict?
    nics = salt.utils.odict.OrderedDict()
    current_nic = salt.utils.odict.OrderedDict()
    no_names = True
    for idx, item in enumerate(net_data, start=0):
        network_key_prefix = _network_key_prefix(lxc_version, idx)
        ...
        if item[0] == '{0}.type'.format(network_key_prefix):
            current_nic = salt.utils.odict.OrderedDict()
        if item[0] == '{0}.name'.format(network_key_prefix):
            no_names = False
...
class _LXCConfig(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.lxc_version = version()
        self.lxc_net_key = _network_key_prefix(lxc_version=self.lxc_version)
        self.data = []
        if self.name:
        ...
        for key, val in six.iteritems(default_data):
            _replace(key, val)
        old_net = self._filter_data(self.lxc_net_key)
        net_datas = _network_conf(lxc_version=self.lxc_version, conf_tuples=old_net, **kwargs)
        if net_datas:
...
def info(name, path=None):
    ...
    try:
        ...
        network_key = _network_key_prefix(version())
        net_type_pattern = re.compile(r"lxc\.network\.type|lxc\.net\.\d+\.type")
        net_prefix_pattern = re.compile(r"lxc\.network\.|lxc\.net\.\d+\.")
        ifaces = []
        current = {}
        for key, val in config:
            if net_type_pattern.match(key):
                current = {'type': val}
                ifaces.append(current)
            elif not current:
                continue
            elif key.startswith(network_key):
                current[re.sub(net_prefix_pattern, '', key)] = val
        if ifaces:
            ret['nics'] = ifaces
...
def edit_conf(conf_file,
    ...
    lxc_version = version()
    network_key = _network_key_prefix(lxc_version)
    ...
    for lxc_kws in lxc_config + [kwargs]:
        ...
        for kwarg in [a for a in lxc_kws]:
            if kwarg.startswith('__'):
                continue
            if kwarg.startswith(network_key):
                net_params[kwarg] = lxc_kws[kwarg]
                lxc_kws.pop(kwarg, None)
        if net_params:
            net_config.append(net_params)
    nic_opts = salt.utils.odict.OrderedDict()
    for idx, params in enumerate(net_config, start=0):
        net_key = _network_key_prefix(lxc_version, idx)
        dev = params.get('{0}.name'.format(net_key), DEFAULT_NIC)
        dev_opts = nic_opts.setdefault(dev, salt.utils.odict.OrderedDict())
        for param in params:
            opt = param.replace('{0}.'.format(net_key), '')
...
def apply_network_profile(name, network_profile, nic_opts=None, path=None):
    ...
    old_net = lxcconfig._filter_data(_network_key_prefix(lxc_version=lxcconfig.lxc_version))
