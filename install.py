...
def install():
    ...
    common.assert_ret_code("/tmp/bootstrap-salt.sh -U -x python3 -p python3-pip -p rustc -p libssl-dev")  # consider: -p libgit2-dev
...
def lxc_install(name: str, autostart: bool, ifc: str, requirements: str, **kwargs):
    ...
    log.info(f"Installing into LXC container, will attach to: {name}, autostart: {autostart}, detected interface: {ifc}")
